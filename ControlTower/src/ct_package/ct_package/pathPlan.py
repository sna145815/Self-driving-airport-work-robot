...
class navCommandSubscriber(Node):
    def __init__(self):
        super().__init__('pathPlan_subscriber')
    ...
    def cmd_callback(self, msg):
        print("data : ", msg.data)
        print("node status_cmd_callback : ", self.node[6])
        data_list = msg.data.split('/')
        
        self.moveCmd = data_list[0]
        self.endPoint = data_list[1]

        if (self.moveCmd == "배차") or (self.moveCmd == "배달") or (self.moveCmd == "복귀"):
            # 임무 할당 플래그 set하기
            if self.movingFlg == 0:
                self.movingFlg = 1
    ...
    def nav_callback(self):
        print("node status_nav_callback : ", self.node[6])
        ...
        try:
            ...
            if self.moveCmd == "배차":
                ...
                if self.endPoint == "S1":
                    print(self.moveCmd, self.endPoint)
    ...
    def gotoGoal(self, pathDict, xyDict, startPoint, endPoint):  
        print("node status_gotoGoal : ", self.node[6])
...
def main(args=None):
    ...
    subInst = navCommandSubscriber()
    rclpy.spin(subInst)
    subInst.run()

    subInst.destroy_node()
    rclpy.shutdown()
